...
export default function DashboardPage() {
  ...
  const filtered = mockDeliverables.filter(
    (d) =>
    ...
    <main className="bg-white min-h-screen py-10 px-4 sm:px-8">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <header className="space-y-1">
          <h1 className="text-3xl font-bold text-blue-600">Transformation Office Dashboard</h1>
          <p className="text-sm text-gray-600">
            Monitor program health, track milestones, and oversee deliverable progress across all
            workstreams
          </p>
        </header>
        ...
        <section className="bg-gray-50 p-4 rounded-lg shadow-sm">
          <h2 className="text-sm font-medium text-gray-800 mb-2">Filters</h2>
        ...
        {/* Summary */}
        <section>
          <h2 className="text-sm font-medium text-gray-800 mb-2">Summary</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-blue-50 p-4 rounded shadow text-center">
              <p className="text-xs text-gray-500">Total Tracks</p>
              <p className="text-2xl font-bold text-gray-800">{totalTracks}</p>
            </div>
            <div className="bg-blue-50 p-4 rounded shadow text-center">
              <p className="text-xs text-gray-500">Workstreams</p>
              <p className="text-2xl font-bold text-gray-800">{totalWorkstreams}</p>
            </div>
            <div className="bg-blue-50 p-4 rounded shadow text-center">
              <p className="text-xs text-gray-500">Deliverables</p>
              <p className="text-2xl font-bold text-gray-800">
                {totalDeliverables}
                <span className="text-green-600 text-sm ml-2">{completedCount} completed</span>
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((d) => {
              ...
              const color =
                d.status === 'completed'
                  ? 'green'
                  : d.status === 'overdue' || d.status === 'at-risk'
                    ? 'red'
                    : 'gray'
              ...
              .map((d) => (
                <li key={d.id} className="flex justify-between items-center text-sm">
                  <span>
                    ...
                    <span className="bg-white text-gray-800 text-xs px-1 py-0.5 rounded shadow">
                      {d.status}
                    </span>
                  </span>
